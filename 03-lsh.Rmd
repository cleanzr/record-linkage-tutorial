...
```
\vspace{-.2in}
For $b = 120$, a pair of records with Jaccard similarity $.25$ will have a `r scales::percent(bin_probs[bin_probs$b == 120 & bin_probs$s == .25, "prob"])` chance of being matched as candidates and a pair of records with Jaccard similarity $.75$ will have a `r scales::percent(bin_probs[bin_probs$b == 120 & bin_probs$s == .75, "prob"])` chance of being matched as candidates. 
