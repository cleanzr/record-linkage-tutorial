...
```{r, echo=TRUE}
# formating object
candidates <- as.data.frame(candidates)[, 1:2] # as data frame
candidates$block <- 1 # default all blocks to 1

# identify groups truth
prev_id <- candidates[1, 1] # get the first id
for(i in unique(candidates[-1, 1])) { # go through all other ids
  matches <- candidates[candidates[, 1] == prev_id, 2] # ids that get matched to the first id
  prev_block <- candidates[candidates[, 1] == prev_id, "block"] # block of the previous id
  if(!(i %in% matches)) { # if i is not matched
    candidates[candidates[, 1] == i, "block"] <- unique(prev_block) + 1 # get new block number
    prev_id <- i # update previous id
  } else {
    candidates[candidates[, 1] == i, "block"] <- unique(prev_block) # assign old block number
  } 
}

# get all ids and their blocks uniquely
unique_grp <- rbind(setNames(candidates[, c(1, 3)], c("id", "block")), 
                    setNames(candidates[, c(2, 3)], c("id", "block")))
unique_grp <- unique(unique_grp)

# find records that have no match
no_match <- setdiff(dat$id, unique_grp$id) # these are id's not blocked
no_match <- data.frame(id = no_match,
                       block = (max(unique_grp$block) + 1):(max(unique_grp$block) + length(no_match)))

# get the blocks and ids
unique_grp <- rbind(unique_grp, no_match)
```
