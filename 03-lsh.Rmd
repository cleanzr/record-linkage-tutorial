...
hashed_jaccard <- expand.grid(record1 = seq_len(n),
                       record2 = seq_len(n))
...
The last thing we need is to be able to go from candidate pairs to blocks.

```{r}
# formating object
candidates <- as.data.frame(candidates)[, 1:2]
candidates$block <- 1

# identify groups truth
prev_id <- candidates[1, 1]
for(i in unique(candidates[-1, 1])) {
  matches <- candidates[candidates[, 1] == prev_id, 2]
  if(!(i %in% matches)) {
    candidates[candidates[, 1] == i, "block"] <- unique(candidates[candidates[, 1] == prev_id, "block"]) + 1
    prev_id <- i
  } else {
    candidates[candidates[, 1] == i, "block"] <- unique(candidates[candidates[, 1] == prev_id, "block"])
  } 
}

# each record that has a match uniquely with its group
unique_grp <- rbind(setNames(candidates[, c(1, 3)], c("id", "block")), 
                    setNames(candidates[, c(2, 3)], c("id", "block")))
unique_grp <- unique(unique_grp)

# add in record that have no match
no_match <- setdiff(dat$id, unique_grp$id)

# get the blocks and ids
unique_grp <- rbind(unique_grp,
      data.frame(id = no_match,
