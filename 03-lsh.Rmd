...
- These are integers, they will take less space which will be useful if we have large documents
- Instead of performing the pairwise Jaccard similarities on the strings, we can perform them on the hashes

```{r, echo=TRUE}
# instead store hash values (less memory)
hashed_shingles <- lapply(shingles, hash_string)
```
