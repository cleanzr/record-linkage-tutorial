...
lam.gs <- rl.gibbs(file.num = file.num, # number of the file
                    X.s = X.s, X.c = X.c, # data 
                    num.gs = 5000, # number of iterations
                    a = a, b = b, c = c, d = d, M = 500) # hyperparams
```

# Output

```{r get-estimates-sizes, echo=TRUE}
# count how many unique latent individuals
size_est <- apply(lam.gs , 1, function(x) { length(unique(x)) })
```

```{r plots, fig.width=3, fig.show='hold'}
ggplot() +
  geom_line(aes(1:5000, size_est)) +
  xlab("Iteration") +
  ylab("Estimated size")

ggplot() +
  geom_histogram(aes(size_est[-(1:100)])) +
...
est_links_pair <- pairwise(links(lam.gs[-(1:100), ]))
